...
//
// Visualizes WHEN and WHERE prices need to be for the wizard to fire.
//
...
function SpreadVsThresholdChart({ state }: { state: DashboardState }) {
  ...
  const fires = (state.recentFires ?? []).filter((f) => {
    ...
    return t >= cutoffMs && (f.status === 'complete' || f.status === 'pending_kraken_deposit' || f.status === 'pending_dotswap');
  });
  ...
  const labelTimes = [0, Math.floor(buckets.length / 2), buckets.length - 1].map(
    (i) => buckets[i]!.at,
  );
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="bg-white border-3 border-wizard-black rounded-[14px_4px_14px_4px] shadow-[3px_3px_0_#040104] p-4 md:p-6">
          ...
          >
            ...
            {/* X-axis time labels */}
            {labelHours.map((h, i) => {
              const idx = [0, Math.floor(buckets.length / 2), buckets.length - 1][i]!;
